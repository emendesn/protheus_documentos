...
//ÀÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÙ
DEFINE MSDIALOG oDlg TITLE "Etiqueta de LAUDO v1.9" FROM 0,0 TO 210,420 OF oDlg PIXEL
...
@ 045,025 MSGET cIMEI PICTURE "@!" 	SIZE 080,010 Valid u_implaudo(cImei) PIXEL OF oDlg

//ÚÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄ¿
//³ Botoes da MSDialog                                                     ³
//ÀÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÙ
@ 080,140 BUTTON "&Cancelar" SIZE 36,16 PIXEL ACTION oDlg:End()
@ 080,040 BUTTON "&Setup" SIZE 36,16 PIXEL ACTION PERGUNTE(CPERG,.t.)
...
if _lAchou
	...
	if apmsgyesno('Imprime a etiqueta de Laudo?')
		if mv_par01 == 2 // argox
			...
			//Titulos
			//MSCBSAY(35 ,10,"Relatorio de Servico"	,_cDirecao ,_cFon ,_aTam[1]) // Linha 01 / Coluna 03 - Cartao de Postagem
			MSCBSAY(10 ,100,"Relatorio de Servico"	,"N","3","01,01")
			MSCBSAY(62 ,100,_cData					,"N","2","01,01")
			MSCBSAY(05 ,095,_cIMEI					,"N","2","01,01")
			MSCBSAY(50 ,095 ,_cModelo 				,"N","2","01,01")
			MSCBSAY(05 ,090 ,_cCliente				,"N","2","01,01")
			MSCBSAY(05 ,085,"Sintomas: "	 		    ,"N","2","01,01")
			...
			if len(_aSintom) > 0
				...
				for x := 1 to len(_aSintom)
				   ...
				   If !_ldescgrd
				        MSCBSAY(20,LINHA ,"- "+alltrim(_aSintom[x])	,"N","2","01,01")
		           Else
    		            IF x = 1
    		               MSCBSAY(20,LINHA,"- "+left(alltrim(_aSintom[x]),_ntamc1)		    ,"N","2","01,01")
    		               LINHA := LINHA - _nlin
       		               MSCBSAY(20,LINHA ,substr(alltrim(_aSintom[x]),_ntamc2)	,"N","2","01,01")    		            
    		            Else
    		               MSCBSAY(20,LINHA ,"- "+left(alltrim(_aSintom[x]),_ntamc1)	,"N","2","01,01")
       		               LINHA := LINHA - _nlin
       		               MSCBSAY(20,LINHA ,substr(alltrim(_aSintom[x]),_ntamc2)	,"N","2","01,01")    		            
			...
			MSCBSAY(05,75 ,"Defeitos: "	,"N","2","01,01")
			if len(_aDefeit) > 0
				LINHA := 75
				_ntamc1:= 40                                       
				_ntamc2:= 41                                       
				for y := 1 to len(_aDefeit)                    
				   ...
				   If !_ldescgrd
				        MSCBSAY(20,LINHA ,"- "+alltrim(_aDefeit[y])	,"N","2","01,01")
		           Else
    		            IF x = 1
    		               MSCBSAY(20,LINHA,"- "+left(alltrim(_aDefeit[y]),_ntamc1)		    ,"N","2","01,01")
       		               LINHA := LINHA - _nlin
       		               MSCBSAY(20,LINHA ,substr(alltrim(_aDefeit[y]),_ntamc2)	,"N","2","01,01")    		            
    		            Else
    		               MSCBSAY(20,LINHA ,"- "+left(alltrim(_aDefeit[y]),_ntamc1)	,"N","2","01,01")
       		               LINHA := LINHA - _nlin
       		               MSCBSAY(20,LINHA,substr(alltrim(_aDefeit[y]),_ntamc2)	,"N","2","01,01")    		            
			...
			MSCBSAY(05,65,"Solucoes: "	,"N","2","01,01")
			if len(_aSoluca) > 0
				LINHA := 65
				_ntamc1:= 40                                       
				_ntamc2:= 41                                       
				...
				for w := 1 to len(_aSoluca)
				   ...
				   If !_ldescgrd
				        MSCBSAY(20,LINHA ,"- "+alltrim(_aSoluca[w])  ,"N","2","01,01")
		           Else
    		            IF x = 1
    		               MSCBSAY(20,LINHA,"- "+left(alltrim(_aSoluca[w]),_ntamc1)	        ,"N","2","01,01")
       		               LINHA := LINHA - _nlin
       		               MSCBSAY(20,LINHA ,substr(alltrim(_aSoluca[w]),_ntamc2) ,"N","2","01,01")    		            
    		            Else
    		               MSCBSAY(20,LINHA-_nlin ,"- "+left(alltrim(_aSoluca[w]),_ntamc1)	 ,"N","2","01,01")
       		               LINHA := LINHA - _nlin
       		               MSCBSAY(20,LINHA ,substr(alltrim(_aSoluca[w]),_ntamc2)	 ,"N","2","01,01")    		            
    		            Endif   
			...
			if len(_aPecas) > 0
				...
				for z := 1 to len(_aPecas)
				   ...
				   If !_ldescgrd
				        MSCBSAY(20,LINHA ,"- "+alltrim(_aPecas[z])	,"N","2","01,01")
		           Else
    		            IF x = 1
    		               MSCBSAY(20,LINHA,"- "+left(alltrim(_aPecas[z]),_ntamc1)			,"N","2","01,01")
       		               LINHA := LINHA - _nlin
       		               MSCBSAY(20,LINHA ,substr(alltrim(_aPecas[z]),_ntamc2)	,"N","2","01,01")    		            
    		            Else
    		               MSCBSAY(20,LINHA-_nlin ,"- "+left(alltrim(_aPecas[z]),_ntamc1)	,"N","2","01,01")
       		               LINHA := LINHA - _nlin
       		               MSCBSAY(20,LINHA ,substr(alltrim(_aPecas[z]),_ntamc2)	,"N","2","01,01")    		            
    		            Endif   
			...
			MSCBSAY(05,40,_cNovModel ,"N","2","01,01")
			
			MSCBSAYBAR(05,25,_cImeiBar 	,ALLTRIM(_cDirecao),"MB07",12,.F.,.T.,.F.,,,,,,,) // Objeto
			//  MSCBSAYBAR(_aCol[1] ,_aLin[11] ,_cImeiBar 	,_cDirecao,"1",12,.F.,.T.,.F.,,,,.T.,,,) // Objeto
			//  MSCBSAYBAR(05 ,20,alltrim(_cImeiBar)	    ,"N","MB07",2.5 ,.F.,.T.,.F.,,2,1) // Objeto
			//	MSCBSAYBAR(_aCol[1],_aLin[2],_aEtiquet[nX, 1],"N","MB07",2.5,.F.,.t.,.F.,,2,1)
			
			MSCBEND()
			
			MSCBCLOSEPRINTER()
			
		else  // zebra nao testada
...
PutSX1(cPerg,"01","Impressora","Impressora","Impressora","mv_ch1","N",01,0,0,"C","",""	,"",,"mv_par01","ZEBRA"	,"","","","ARGOX"		,"","",""	,"","","","","","","","")

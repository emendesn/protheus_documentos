...
Private cString 	 := "SF2"
Private aOrd 		 := {}
Private cDesc1       := "Serใo Impressas Notas Anteriores เ 01/09/09"    // Alterado Solicita็ใo Carlos
Private cDesc2       := " "
...
Private _xImpSt     := GetMV("MV_XSTBSVA")
Private _xSubSt     := GetMV("MV_XSUBSTN")
Private _xImport    := GetMV("MV_XTESIMP")
Private _lbonif    :=.F.
...
//	_cQuery += "        F1_ICMSRET 'ICMSRET', F1_BRICMS  'BRICMS'                                              , "                                         "
	...
	_cQuery += "        AND F1_DOC    BETWEEN '"+MV_PAR01+"' AND '"+MV_PAR02+"' "
 	_cQuery += "        AND F1_EMISSAO < '20090901' " 
...
//	_cQuery += "        F2_TRANSP  'TRANSP' , F2_PLIQUI  'PLIQUI' , F2_PBRUTO  'PBRUTO'                        , "
	...
	_cQuery += "        PRODUTO = CASE WHEN B1_DESC = '' THEN B1_DESC ELSE B1_DESC END, "
	...
	_cQuery += "        AND F2_SERIE  = '"+MV_PAR03+"' "
  	_cQuery += "        AND F2_EMISSAO < '20090901' " 
...
while QRY->(!eof())
	...
	//CLAUDIA 10/08/09 NF COMPLEMENTO DE IPI NAO PODE SOMAR IPI NO TOTAL DA NF
	if _nValFat == 0 .or.( QRY->TIPO = 'P' .AND. QRY->VALMERC = QRY->VALIPI) // COMPLEMENTO DE IPI
	   _nValFat := QRY->VALMERC  + QRY->SEGURO + QRY->FRETE + QRY->DESPESA + QRY->ICMSRET - QRY->DESCONTO		
	endif                       		
	/*  ----  */ 
	...
	Else // Claudia 20/12/2008 NF DE IMPORTACAO TEM QUE SOMAR OS IMPOSTOS/DESPESAS NA NF 
		IF MV_PAR04 <> 2 // NF DE ENTRADA
			IF QRY->TES $ ALLTRIM(_xImport) // '310/318/322' // IMPORTACAO --INCLUSO TES 318 - Edson Rodrigues - 02/03/09, claudia 14/08/2009
...
//	_nVlIPI		:= QRY->VALIPI
	...
	IF MV_PAR04 <> 2 // NF DE ENTRADA 
		IF QRY->TES $ _xImport // IMPORTACAO
	...
	IF MV_PAR04 == 2 // NF DE SAIDA 
		...
		If QRY->TES $ alltrim(_xImpSt) // '811/812/822/839/841/831/843' zerar a Base e o valor da Sub. Tributaria
			// CLAUDIA 10/08/09 , colocado parametro para nใo ter que alterar o programa sempre
			_nBsIcmsSt := 0
			_nVlIcmsSt := 0                         
			IF QRY->TIPO $ "DB" 
				_nValFat   := _nValFAt - QRY->ICMSRET
			Endif
			If QRY->TES $ alltrim(_xSubSt) //  '822/839/841/831'  -- subtrai o ICMS RETIDO DO TOTAL DA NF 
				// CLAUDIA 10/08/09 , colocado parametro para nใo ter que alterar o programa sempre				
				_nValFat   := _nValFAt - QRY->ICMSRET
			Endif
		endif
...
aAdd(aRegs,{cPerg,"02","Ate Nota Fiscal    ?","","","mv_ch2","C",09,0,0,"G","","mv_par02","","","","","","","","","","","","","","","","","","","","","","","","",""})
...
If INT(_nBsICMS) = INT(_nValMerc) // acerto de decimais Claudia 20/06/2009
	_nValMerc := _nBsICMS	
ENDIF
...
if _nZFPis > 0
	//@ nlin, 022 psay "Desconto de PIS (aliq. "+Transform(_nZFPis / _nZFMerc * 100,"@E 99.99")+"%): R$ " + alltrim(Transform(_nZFPis ,"@E 999,999.99")) claudia 11/11/2009 - denilza pediu para fixar a aliquota
	@ nlin, 022 psay "Desconto de PIS (aliq. "+Transform(1.65,"@E 99.99")+"%): R$ " + alltrim(Transform(_nZFPis ,"@E 999,999.99"))
	_nTotSufr += _nZFPis
	nLin++
endif
if _nZFCof > 0
	//@ nlin, 022 psay "Desconto de COFINS (aliq. "+Transform(_nZFCof / _nZFMerc * 100,"@E 99.99")+"%): R$ " + alltrim(Transform(_nZFCof ,"@E 999,999.99")) claudia 11/11/2009 - denilza pediu para fizar a aliquota
	@ nlin, 022 psay "Desconto de COFINS (aliq. "+Transform(7.60,"@E 99.99")+"%): R$ " + alltrim(Transform(_nZFCof ,"@E 999,999.99"))
	_nTotSufr += _nZFCof
	nLin++
endif
if _nTotSufr > 0

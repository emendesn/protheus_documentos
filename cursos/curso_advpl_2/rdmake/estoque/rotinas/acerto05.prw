...
cQry := "    SELECT	'SA2 - FORNECEDOR' CADASTRO, '012' COD_ERR,'2 VIRGULAS NO ENDEREÇO' OCORRENCIA, A2_COD, A2_LOJA, A2_NOME, A2_END, A2_BAIRRO, A2_MUN, A2_CEP, A2_EST ,'' , '', '', '', A2_INSCR INS_EST, A2_COD_MUN COD_MUN, A2_CGC CGC, R_E_C_N_O_ " +CRLF
cQry += "    FROM   SA2020 " +CRLF
cQry += "   WHERE A2_END LIKE '%,%,%' AND D_E_L_E_T_ <> '*'   AND A2_MSBLQL <> 1 " +CRLF

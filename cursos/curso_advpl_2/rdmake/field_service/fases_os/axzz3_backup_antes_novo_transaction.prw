...
If Empty(Posicione("SX3",1,cString,"X3_ARQUIVO"))
	Help("",1,"","NOX3X2IX","NรO ษ POSSอVEL EXECUTAR, FALTA"+ENTER+"X3, X2, IX E X7",1,0)
	RETURN
Endif
...
Do Case
	...
	Case _cTab == "FAS2"
		...
		ElseIf Empty(FAS2) //Adcionado Edson Rodrigues - 17/03/09
			If _lEncOs .And. left(CODSETORI,6)<>left(CODSE2ORI,6)
				...
				Endif
			ElseIf  !_lEncOs
				apMsgStop("Favor informar uma fase destino, nใo ้ permitido apontar fase destino em branco quando a fse oriem nใo ecerra OS. Favor informar a fase destino.","Fase Destino invแlida")
				_lRet := .F.
	...
	Case _cTab == "IMEI"
		...
		If !Empty(IMEI) .And. !Empty(CODTEC)
			...
			If _lContinua .AND. GETMV("BH_VALMAP",.F.,.T.) .AND. ! EMPTY(ZZ4->ZZ4_ETQMEM) .AND. ZZ4->ZZ4_STATUS=='3'  // Parametro Para Validar Existencia da Etiqueta Master com o Documento planejado para Produ็ใo)
...
//					ZZ4->ZZ4_TRANSC := iif(empty(ZZ4->ZZ4_TRANSC) .and. !empty(ZZ3->ZZ3_TRANSC), ZZ3->ZZ3_TRANSC, ZZ4->ZZ4_TRANSC)
...
// Cria Componentes Padroes do Sistema
If  AllTrim(_cfase1) $ AllTrim(cfasouinf) .And. _ctela='1'
...
If ALLTRIM(_cfase1) $ AllTrim(cfastnfix) .And. _ctela='2'
...
If LEN(_aRecnoZZ3[_ni,2]) > 0
	...
	Else
		...
		Else
			...
			_laponta:=.F.
			// apMsgStop("Processo Refurbish Sony - Houve Falha no empenho das pe็as da OP : "+AllTrim(LEFT(ZZ4->ZZ4_OS,6))+ " Favor empenhar a(s) peca(s) nao apontada(s). Duvidas Contate o administrador do sistema. Falha no apontamento.")
			_lContinua:=.F.
		EndIf
	EndIf
Else

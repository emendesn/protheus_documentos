...
±±ºPrograma  ³MCLAIMS   º Autor ³ Edson Rodrigues    º Data ³  05/05/09   º±±
±±ÌÍÍÍÍÍÍÍÍÍÍØÍÍÍÍÍÍÍÍÍÍÊÍÍÍÍÍÍÍÏÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÊÍÍÍÍÍÍÏÍÍÍÍÍÍÍÍÍÍÍÍÍ¹±±
±±ºDescricao ³ Gera arquivo MCLAIMS - Motorola coma as OS dos             º±±
±±º          ³ Processo Nextel                                            º±±
...
User Function Mclaims()
	...
	Private _dtcortf02 :=CTOD('18/04/2013') // Primeira mudanca de Alphaville para Osasco
	Private _dtcor2f02 :=CTOD('01/04/2015') // Segunda  mudanca de Osasco para Alphaville // M.Munhoz - 06/08/2015 - GLPI 23579
//	Private _cProble
//	Private _cRepair                                                                           
...
Static Function OkGeraTxt
	...
	//ÀÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÙ
    Private cArqTxt := "\Relato\MClaims\" + Alltrim(mv_par05) + ".TXT" 
...
Local _aMCLEnv := {}
   ...
   IF Len(aoperac) > 0
       For nop:=1 to Len(aoperac)
          // M.Munhoz - 06/08/2015 - GLPI 23579
          IF (right(cNumEmp,2) ='06' .and. MV_PAR03>=_dtcortf02 .and. MV_PAR03<_dtcor2f02) .OR. ;
             (right(cNumEmp,2) ='02' .and. MV_PAR03< _dtcortf02) .or. ; 
             (right(cNumEmp,2) ='02' .and. MV_PAR03>=_dtcor2f02) .or. ; 
             (right(cNumEmp,2) ='02' .and. aoperac[nop] $ "N01/N10/N11" )
...
/* SISTEMA FICAVA MUITO LENTO PRA RETRONAR E ALTERAMOS A QUERY - HUDSON DE SOUZA SANTOS 23/04/2014	                            
...
//  _cJoin   += CR + " AS SZA ON ZA_FILIAL=ZZ4_FILIAL AND ZA_CLIENTE=ZZ4_CODCLI AND ZA_LOJA=ZZ4_LOJA AND ZA_NFISCAL=ZZ4_NFENR AND ZA_SERIE=ZZ4_NFESER AND ZA_IMEI=ZZ4_IMEI "
    ...
    _cJoin   += CR + "               SELECT ZA.ZA_FILIAL,ZA_CLIENTE,ZA_LOJA,ZA.ZA_NFISCAL,ZA_SERIE,ZA.ZA_IMEI,"

//ÚÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄ¿
//³Alterado 14/03/2014 - GLPI XXXXX                     ³
//³Apresentar no campo de NF de compra as informações da³
//³NF de troca caso atenda por elas.                    ³
//ÀÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÙ
//	_cJoin   += CR + "                      ZA_NFCOMPR,ZA_DTNFCOM,"
	_cJoin   += CR + " CASE WHEN isnull(ZA_GARNFT,'') = 'S' THEN ZA_NFTROCA ELSE ZA_NFCOMPR END as ZA_NFCOMPR,"
	_cJoin   += CR + " CASE WHEN isnull(ZA_GARNFT,'') = 'S' THEN ZA_DTNFTRO ELSE ZA_DTNFCOM END as ZA_DTNFCOM,"

	_cJoin   += CR + "                      ZA_CODRECL,ZA_CODTRAN,SZA2.ZA_CODAUTO,ZA_TRACKIN,SZA2.ZA_OSAUTOR,ZA_INFACES,"
...
//    Procregua(_nReg)
//    REG->(dbCloseArea())  
	_cQuery := CR + "IF object_id('tempdb..##MCLAIM_ZZ3_TEMP') IS NOT NULL DROP TABLE ##MCLAIM_ZZ3_TEMP"
	_cQuery += CR + "SELECT"
	_cQuery += CR + "	ZZ3_FILIAL,ZZ3_LAB,ZZ3_NUMOS,ZZ3_IMEI,ZZ3_SEQ,ZZ3_STATUS,ZZ3_FASE2,ZZ3_CODSE2,ZZ3_CODTEC,ZZ3_DATA,ZZ3_HORA,ZZ3_ENCOS,ZZ3_ARTIME, ZZ3_MINUMB,ZZ3_DEFINF"
	_cQuery += CR + "INTO ##MCLAIM_ZZ3_TEMP"
	_cQuery += CR + "FROM "+RetSQLName("ZZ3")+" ZZ3_1 (NOLOCK)"
	_cQuery += CR + "INNER JOIN ("
	_cQuery += CR + "	SELECT"
	_cQuery += CR + "		MAX(R_E_C_N_O_) AS RECNO"
	_cQuery += CR + "	FROM "+RetSQLName("ZZ3")+" (NOLOCK)"
	_cQuery += CR + "	WHERE ZZ3_ENCOS = 'S'"
	_cQuery += CR + "		AND D_E_L_E_T_=''"
	_cQuery += CR + "		AND ZZ3_DATA >= '"+DtoS(MV_PAR01)+"' AND ZZ3_DATA <= '"+DtoS(MV_PAR02)+"'"
	_cQuery += CR + "		AND ZZ3_STATUS = '1'"
	_cQuery += CR + "		AND ZZ3_ESTORN <> 'S'"
	_cQuery += CR + "		AND ZZ3_LAB IN " + _cLab
	_cQuery += CR + "		AND ZZ3_FILIAL = '"+xFilial("ZZ3")+"'"
	_cQuery += CR + "	GROUP BY ZZ3_FILIAL,ZZ3_IMEI,ZZ3_NUMOS"
	_cQuery += CR + ") AS ZZ3_2 ON ZZ3_1.R_E_C_N_O_ = ZZ3_2.RECNO"
	_cQuery += CR + "WHERE D_E_L_E_T_ = ''"
	_cQuery += CR + "	AND ZZ3_FILIAL = '"+xFilial("ZZ3")+"'"
	_cQuery += CR + "	AND ZZ3_ESTORN <> 'S'"
	_cQuery += CR + "	AND ZZ3_STATUS = '1'"
	_cQuery += CR + "	AND NOT(ZZ3_FASE1 = '39' AND ZZ3_FASE2 = '15' AND ZZ3_CODSET = '000005')"
	TcSqlExec(_cQuery)
	_cQuery := CR + "SELECT"
	_cQuery += CR + "	ZZ4_CODCLI, ZZ4_LOJA,"
	_cQuery += CR + "	CASE WHEN ZA_NFCOMPR IS NULL THEN '' ELSE ZA_NFCOMPR END as NFCOMPR,"
	_cQuery += CR + "	CASE WHEN ZA_DTNFCOM IS NULL THEN '' ELSE ZA_DTNFCOM END as DTNFCOM,"
	_cQuery += CR + "	CASE WHEN ZG.ZZG_RECCLI  IS NULL THEN '' ELSE ZG.ZZG_RECCLI END as CODRECL,"
	_cQuery += CR + "	CASE WHEN ZA_DEFRECL IS NULL THEN '' ELSE ZA_DEFRECL END as DEFRECL,"
	_cQuery += CR + "	CASE WHEN ZA_CODTRAN IS NULL THEN '' ELSE ZA_CODTRAN END as CODTRAN,"
	_cQuery += CR + "	CASE WHEN A1_XCODSAM <> ''		THEN A1_XCODSAM"
	_cQuery += CR + "		 WHEN ZA_CODAUTO IS NULL	THEN ''"
	_cQuery += CR + "	ELSE ZA_CODAUTO END as CODAUTO,"
	_cQuery += CR + "	CASE WHEN ZA_TRACKIN IS NULL THEN '' ELSE ZA_TRACKIN END as TRACKIN,"
	_cQuery += CR + "	CASE WHEN ZZ4_OSAUTO IS NULL THEN '' ELSE ZZ4_OSAUTO END as OSAUTOR,"
	_cQuery += CR + "	CASE WHEN ZA_INFACES IS NULL THEN '' ELSE ZA_INFACES END as INFACES,"
	_cQuery += CR + "	CASE WHEN ZA_CTRLACR IS NULL THEN '' ELSE ZA_CTRLACR END as CTRLACR,"
	_cQuery += CR + "	CASE WHEN ZA_CPFCLI IS NULL THEN '' ELSE ZA_CPFCLI END as CPFCLI,"
	_cQuery += CR + "	CASE WHEN ZA_CNPJNFC IS NULL THEN '' ELSE ZA_CNPJNFC END as CNPJNFC,"
	_cQuery += CR + "	CASE WHEN ZA_SERNFC IS NULL THEN '' ELSE ZA_SERNFC END as SERNFC,"
	_cQuery += CR + "	CASE WHEN ZA_SPC IS NULL THEN '0' ELSE ZA_SPC END as SPCPROJ,"
	_cQuery += CR + "	'BR5360' + LEFT(ZZ4_OS,6) AS 'MASC_CODE',"
	_cQuery += CR + "	'' AS 'TRANS_CODE',"
	_cQuery += CR + "	ZZ4_OS, ZZ4_TRANSC, ZZ4_SOFIN, ZZ4_SOFOUT, ZZ4_IMEI, ZZ4_SWAP, ZZ4_CARCAC, ZZ4_CODPRO, B1_XCODAPC, B1_CREFDES, B1_NREFDES, ZZ4_NFEDT,"
	_cQuery += CR + "	ZZ4_NFEHR, ZZ4_DOCDTE, ZZ4_DOCHRE, ZZ4_DOCDTS, ZZ4_DOCHRS, ZZ4_NFSNR, ZZ4_PROBLE, ZZ4_REPAIR,  ZZ4_NFSSER, ZZ4_NFSDT, ZZ4_NFSHR, ZZ4_NFSCLI,"
	_cQuery += CR + "	ZZ4_NFSLOJ, ZZ4_MCLGER, ZZ3_CODTEC, ZZ3_DATA, ZZ3_HORA, ZZ3_ENCOS, ZZ3_ARTIME, ZZ3_MINUMB, ZZ3_NUMOS, ZZ3_IMEI, ZZ3_FASE2, ZZ3_CODSE2, ZZ3_LAB,"
	_cQuery += CR + "	ZZ3_DEFINF, ZZ4_ATPRI, ZZ4_ATULT, ZZ4_OPEBGH, ZZ4_GARANT, ZZ4_DEFINF, ZZ4_NFENR as NFENTRADA, ZZ4_NFESER as SERIE, A1.A1_NOME, A1.A1_NREDUZ,"
	_cQuery += CR + "	A1.A1_XCODSAM, ZZ4_CHVFIL"
	_cQuery += CR + "FROM "+RetSQLName("ZZ4")+" as ZZ4 (NOLOCK)"
	_cQuery += CR + "LEFT OUTER JOIN ("
	_cQuery += CR + "	SELECT"
	_cQuery += CR + "		CASE WHEN isnull(ZA_GARNFT,'') = 'S' THEN ZA_NFTROCA ELSE ZA_NFCOMPR END as ZA_NFCOMPR,"
	_cQuery += CR + "		CASE WHEN isnull(ZA_GARNFT,'') = 'S' THEN ZA_DTNFTRO ELSE ZA_DTNFCOM END as ZA_DTNFCOM,"
	_cQuery += CR + "		ZA.ZA_FILIAL, ZA_CLIENTE, ZA_LOJA, ZA.ZA_NFISCAL, ZA_SERIE, ZA.ZA_IMEI, ZA_CODRECL, ZA_CODTRAN, SZA2.ZA_CODAUTO, ZA_TRACKIN, SZA2.ZA_OSAUTOR,"
	_cQuery += CR + "		ZA_INFACES, ZA_CTRLACR, ZA_CPFCLI, ZA_CNPJNFC, ZA_SERNFC, ZA_DEFRECL, ZA_SPC, ZA.ZA_CODPRO"
	_cQuery += CR + "	FROM "+RetSQLName("SZA")+" as ZA (NOLOCK)"
	_cQuery += CR + "	JOIN ("
	_cQuery += CR + "		SELECT"
	_cQuery += CR + "			ZA_FILIAL, ZA_IMEI, ZA_NFISCAL, ZA_CODPRO, MAX(ZA_CODAUTO) as ZA_CODAUTO, MAX(ZA_OSAUTOR) as ZA_OSAUTOR, MAX(R_E_C_N_O_) as SZARECNO"
	_cQuery += CR + "		FROM "+RetSQLName("SZA")+" as ZA (NOLOCK)"
	_cQuery += CR + "		WHERE ZA.D_E_L_E_T_ = '' AND ZA_FILIAL = '"+xFilial("SZA")+"'"
	_cQuery += CR + "		GROUP  BY ZA_FILIAL, ZA_IMEI, ZA_NFISCAL, ZA_CODPRO"
	_cQuery += CR + "		) AS SZA2 ON ZA.R_E_C_N_O_ = SZA2.SZARECNO"
	_cQuery += CR + "	WHERE ZA.D_E_L_E_T_=''"
	_cQuery += CR + "		AND ZA.ZA_FILIAL = '"+xFilial("SZA")+"'"
	_cQuery += CR + ") AS SZA ON ZA_FILIAL = ZZ4_FILIAL AND ZA_CLIENTE = ZZ4_CODCLI AND ZA_LOJA = ZZ4_LOJA AND ZA_NFISCAL = ZZ4_NFENR AND ZA_SERIE = ZZ4_NFESER"
	_cQuery += CR + "			AND ZA_IMEI=ZZ4_IMEI AND ZA_CODPRO = ZZ4_CODPRO"
	_cQuery += CR + "INNER JOIN ( SELECT B1_COD,B1_XCODAPC,B1_CREFDES,B1_NREFDES FROM "+RetSQLName("SB1")+" as B1 (NOLOCK) WHERE B1.D_E_L_E_T_='' AND B1_FILIAL='"+xFilial("SB1")+"') AS SB1 ON B1_COD=ZZ4_CODPRO"
	//_cQuery += CR + "INNER JOIN ##MCLAIM_ZZ3_TEMP as ZZ3 ON ZZ3_FILIAL = ZZ4_FILIAL AND LEFT(ZZ3_IMEI,15)=LEFT(ZZ4_IMEI,15) AND LEFT(ZZ3_NUMOS,6) = LEFT(ZZ4_OS,6)"
	_cQuery += CR + "INNER JOIN ##MCLAIM_ZZ3_TEMP as ZZ3 ON ZZ3_FILIAL = ZZ4_FILIAL AND ZZ3_IMEI=ZZ4_IMEI AND LEFT(ZZ3_NUMOS,6) = LEFT(ZZ4_OS,6)"
	_cQuery += CR + "INNER JOIN SA1020 as A1 ON A1.D_E_L_E_T_ = '' AND A1.A1_FILIAL = '"+xFilial("SA1")+"' AND A1.A1_COD = ZZ4.ZZ4_CODCLI AND A1.A1_LOJA = ZZ4.ZZ4_LOJA"
	_cQuery += CR + "LEFT OUTER JOIN ZZG020 as ZG (NOLOCK) ON ZZG_FILIAL = '"+xFilial("SZG")+"' AND ZZ4.ZZ4_DEFINF = ZG.ZZG_CODIGO AND ZG.ZZG_LAB = '2' AND ZG.D_E_L_E_T_ = ''"
	_cQuery += CR + "WHERE ZZ4.D_E_L_E_T_ = ''"
	_cQuery += CR + "	AND ZZ4_FILIAL = '"+xFilial("ZZ4")+"'"
    If MV_PAR07 == 1
		_cQuery += CR + " AND LEFT(ZA_SPC,3) = 'SPC'"
    ElseIf MV_PAR07 == 2
		_cQuery += CR + " AND ISNULL(SZA.ZA_SPC,'0') IN ('0','')"
    EndIf
	_cQuery += CR + "	AND ZZ4_STATUS >= '5'"
	_cQuery += CR + "	AND ZZ4_NFSDT >= '"+DtoS(MV_PAR03)+"' AND ZZ4_NFSDT <= '"+DtoS(MV_PAR04)+"'"
	_cQuery += CR + "	AND ZZ4_OPEBGH IN " + coperac
	_cQuery += CR + "	AND ZZ4_NFSTES NOT IN ('798','880')"
	If !Empty(MV_PAR06)
		_cQuery += CR + "	AND '" + AllTrim(MV_PAR06) + "' like '%'+RTRIM(ZZ4.ZZ4_OS)+'%' AND ZZ4.ZZ4_OS <> ''"
	Endif
	_cQuery += CR + "ORDER BY ZZ3_IMEI,ZZ3_NUMOS,ZZ3_DATA, ZZ3_HORA"
	_cQuery += CR + "DROP TABLE ##MCLAIM_ZZ3_TEMP"
	...
	If QRY->(!EOF()) 
		...
		_c1_3 := _c1_7 := _c1_8 := _cendcons3 := _cendcons4 := ""
            ...
            IF empty(ctransc) .or. empty(csoftout) .or. empty(_ccodrecl)    // Alterado condição - Edson Rodrigues 23/03/10
              ...
              qryzz3("3",LEFT(QRY->ZZ3_NUMOS,6),QRY->ZZ3_IMEI,cfastnfix,csettnfix,cfasouinf, AllTrim(QRY->ZZ4_TRANSC),AllTrim(QRY->ZZ4_SOFIN),AllTrim(QRY->ZZ4_SOFOUT)) 	          
            ...
            Elseif cinftrans='S' .and. ctransfix = "N"
	           ...
	           qryzz3("1",LEFT(QRY->ZZ3_NUMOS,6),QRY->ZZ3_IMEI,cfastnfix,csettnfix,cfasouinf, AllTrim(QRY->ZZ4_TRANSC),AllTrim(QRY->ZZ4_SOFIN),AllTrim(QRY->ZZ4_SOFOUT)) 
		    Endif  
		    
		    dbselectarea("QRY")
		  
		If (!empty(ctransc) .AND. !(ALLTRIM(ctransc)$ "IOW,IRF,IXT") .OR. !empty(ctransc) .AND. Len(AllTrim(QRY->SPCPROJ))>1)
		       ...
		       qryzz3("2",LEFT(QRY->ZZ3_NUMOS,6),QRY->ZZ3_IMEI,cfastnfix,csettnfix,cfasouinf, AllTrim(QRY->ZZ4_TRANSC),AllTrim(QRY->ZZ4_SOFIN),AllTrim(QRY->ZZ4_SOFOUT))
		       //qrysz9("1",LEFT(QRY->ZZ3_NUMOS,6),LEFT(QRY->ZZ3_IMEI,15),QRY->ZZ4_OPEBGH,QRY->ZZ3_LAB)
		       qrysz9("1",LEFT(QRY->ZZ3_NUMOS,6),QRY->ZZ3_IMEI,QRY->ZZ4_OPEBGH,QRY->ZZ3_LAB)
		       dbselectarea("QRY")
			  _ccpfclie :=""
		      _cnomcons :=""
		      _cendcons :=""
...
//				aAdd(_aMCLEnv, {QRY->ZZ4_IMEI, QRY->ZZ4_OS})
				...
				aAdd(_aMCLEnv, xFilial("ZZ4")+QRY->ZZ4_IMEI+left(QRY->ZZ4_OS,6) )
...
cimei    :=alltrim(cImei) + space(TamSX3("ZZ4_IMEI")[1]-len(alltrim(cImei)))

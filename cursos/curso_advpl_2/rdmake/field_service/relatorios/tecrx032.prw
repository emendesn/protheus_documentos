...
Private _cparm    := ""
Private _tFile    := .t.  
//_APARAMS :={"WF","000001","02","02"}

//u_GerA0003(ProcName())
//ConOut("Parametros start rotina : "+strzero(len(_APARAMS),2)+".")
//for x:=1 to len(_APARAMS) 
//   ConOut("Detalhes dos Parametros start rotina : "+ConvType(_APARAMS[x])+" .")
//next   
...
ELSEIF  LEN(_APARAMS) > 0 .and. _APARAMS[1]="WF"
        _cempres:=_APARAMS[3]
        _cfilial:=_APARAMS[4]
        PREPARE ENVIRONMENT EMPRESA  _cempres FILIAL  _cfilial TABLES "SD1","SD2","SZ1","AB9","AA3","SA1","SB1","SBM","SC6","SX5","SZ1"
        cchavesx5:=_APARAMS[2]
        _APARAMS := {_cempres,_cfilial,DATE(),DATE(),DATE(),DATE(),"1","22/26/27"," ",1,3,0,2,1,.T.,"000169",""}   
        _CCONTEUDO := AllTrim(Posicione("SX5", 1, xFilial("SX5")+'WF'+cchavesx5, "X5_DESCRI"))    

        
        If !empty(_CCONTEUDO)
           _cparam:=alltrim(_CCONTEUDO)
           _ncount:=0
           for x:=1 to len(_cparam)  
           
              ndiv:=AT(",",_cparam)
              If !ndiv = nil .and. ndiv > 0       
                _ncount++
                if _ncount=1
                   _APARAMS[_ncount]:=_cempres
                   _cparam:=substr(_cparam,ndiv+1)
                Elseif _ncount=2
                   _APARAMS[_ncount]:=_cfilial
                   _cparam:=substr(_cparam,ndiv+1)                 
                
                Elseif strzero(_ncount,2) $ "03/04/05/06/10/11/12/13/14/15"
                    _cdata:=substr(_cparam,1,ndiv-1)                 
                   _APARAMS[_ncount]:=&_cdata
                   _cparam:=substr(_cparam,ndiv+1)                 
                
                
                Else
                   _APARAMS[_ncount]:=substr(_cparam,1,ndiv-1)                 
                   _cparam:=substr(_cparam,ndiv+1)                 
               Endif
             Elseif ndiv = 0
               //_ncount++
               //_APARAMS[_ncount]:=alltrim(_cparam)                 
             Endif  
           Next   
        ENDIF
  
        ConOut("Parametros Gerais de execucao da rotina : "+strzero(len(_APARAMS),2)+".")          
Endif
...
If !lBat  //.and. !IsBlind()
	...
	// Ticket 9673 - M.Munhoz - Bloqueio de execucao do cycletime Protheus pelos usuarios possuem Cycletime Excel/SQL.
	// Ticket 23274 - M.Munhoz - Liberacao do relatorio pro Luiz devido a novas operacoes/clientes
	// if upper(alltrim(cUserName)) $ "LUIZ.REIS"
	if upper(alltrim(cUserName)) $ ""
		apMsgStop('Caro usuแrio, solicitamos que o Cycletime seja utilizado atrav้s do recurso em Excel. Qualquer d๚vida ou problema favor contatar o departamento de T.I.','Rotina bloqueada')
		ConOut("Cycletime cancelado. Rotina acionada por usuario que possui Cycletime em Excel.")
		return nil
	else
		cMensagem := "O Relatorio Cycletime estแ sendo gerado pelo usuแrio "+alltrim(cUserName)+" em "+DTOC(date())+" as "+time()+" hs atrav้s do relat๓rio Protheus."+ _center
		Path := "172.16.0.7"
	//  U_ENVIAEMAIL("Cycletime Protheus gerado","edson.rodrigues@bgh.com.br;marcelo_munhoz@terra.com.br","",cMensagem,Path)
		U_ENVIAEMAIL("Cycletime Protheus gerado","vitor.kwon@mfconsulting.com.br","",cMensagem,Path)
	endif
...
Else 
  ...
  mv_par13:=IIF(_APARAMS[17] == NIL,mv_par13,_APARAMS[17])    //  -> Operacoes
//  IF _APARAMS[17] == NIL
//  ALERT('_APARAMS[17] = NIL')
//  ELSE
//  ALERT('_APARAMS[17] DIFERENTE DE NIL')
//  ENDIF
  ...
  For x:=1 to len(_ausers)
     _aArqCYCL   :={}                                   
     //cpath := StrTran(alltrim(_ausers[x][4]), "\", "/" ) //alterado em funcao da mudan็a da plataforma de linux para Windows - Edson Rodrigues - 20/05/2012 
...
While TRB1->(!eof())
	...
	IF mv_par11 = 1 //  Nextel               
	    ...
	    cLinha += TRB1->SPC      			+ ';' 
	Endif
	cLinha += RIGHT(TRB1->DTNFCOM,2)+'/'+SUBSTR(TRB1->DTNFCOM,5,2)+'/'+LEFT(TRB1->DTNFCOM,4)    + ';' //TRB1->DTNFCOM			+ ';'
	cLinha += TRB1->STATUS 			+ ';'
	...
	cLinha += TRB1->COR 			+ ';'
	cLinha += TRB1->MOTSCR 			+ ';'
	cLinha += tabela("YR",TRB1->MOTSCR,.f.) + ';'
...
PutSX1(cPerg,'13','Operacoes'			,'Operacoes'			,'Operacoes'			,'mv_chd','C',20,0,0,'G','',''		,'','S','mv_par13',''		,''			,''			,'',''				,''				,''				,''			,''			,''			,'','','','','','')
...
//Ticket 8117 - M.Munhoz - 17/08/2012 - Alteracao do defeito/solucao do Cycletime para o ZZ4 usando o mesmo criterio do MCLAIM
...
_cqry += CR +  "       'DEFCONST' = ZZ4_PROBLE, " + ENTER
...
_cqry += CR +  "       'SOLCONST' = ZZ4_REPAIR, " + ENTER
...
Endif
_cqry += CR + "        ZA_DTNFCOM 'DTNFCOM', " + ENTER
...
_cqry += CR + "        ZZ4.ZZ4_MOTSCR 'MOTSCR'" + ENTER
...
_cqry += CR + " LEFT JOIN   "+retsqlname("ZZJ")+" AS ZZJ  (nolock) " + ENTER
_cqry += CR + " ON     ZZJ.D_E_L_E_T_ = '' AND ZZJ.ZZJ_OPERA = ZZ4.ZZ4_OPEBGH " + ENTER
...
_cqry += CR + " ON     ZZ1.D_E_L_E_T_ = '' AND ZZ1.ZZ1_FILIAL = '  ' AND ZZ1.ZZ1_LAB = ZZJ.ZZJ_LAB AND ZZ1.ZZ1_FASE1 = ZZ4.ZZ4_FASATU AND ZZ1.ZZ1_CODSET = ZZ4.ZZ4_SETATU " + ENTER
...
_cqry += CR + " ON     ZZ1_1.D_E_L_E_T_ = '' AND ZZ1_1.ZZ1_FILIAL = '  ' AND ZZ1_1.ZZ1_LAB = ZZJ.ZZJ_LAB AND ZZ1_1.ZZ1_FASE1 = ZZ4.ZZ4_FASMAX AND ZZ1_1.ZZ1_CODSET = ZZ4.ZZ4_SETMAX " + ENTER
...
_cqry += CR + "        AND ZZ4.ZZ4_NFSDT BETWEEN '"+dtos(mv_par03)+"' AND '"+dtos(mv_par04)+"' " + ENTER
if !empty(mv_par13) // Caso o parametro seja informado, filtra por OPERACAO (ZZ4_OPEBGH / ZZJ)
	_cqry += CR + "        AND '"+alltrim(MV_PAR13)+"' LIKE '%'+ZZ4.ZZ4_OPEBGH+'%'" + ENTER
endif
If !alltrim(mv_par06) $ carmproc
...
MemoWrite("c:\cycletime_Motorola.sql", _cqry)
...
Local cNovo := ""
local nDec := iif(nDec=nil,0,nDec)

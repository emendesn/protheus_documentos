...
u_GerA0003(ProcName())
		...
		cSql := " Select ZA1_TURNO,SUM(ZA1_HRTOTA)ZA1_HRTOTA " 
		cSql += " From "+RetSQLName("ZA1") 
		cSql += " WHERE ZA1_FILIAL='"+xFilial("ZA1")+"' AND D_E_L_E_T_<> '*' "
		...
		While TMP->(!Eof())
			aAdd(aCaixas,{TMP->ZA1_TURNO,Transform(TMP->ZA1_HRTOTA, "@E 999.99")})
			TMP->(DbSkip())
		EndDo
		...
		@ 016,008 LISTBOX oList1 FIELDS HEADER "Turno","Total Horas" COLSIZES 100,60 SIZE 180,164 OF oGrp PIXEL  
		oList1:SetArray(aCaixas)
		oList1:bLine   := {||{aCaixas[oList1:nAt,1],aCaixas[oList1:nAt,2]}} 
		oList1:aArray := {}
		oList1:Refresh()
		...
		oGerar     := TButton():New( 200,010,"Gerar Calendario"			,oGrp1	,{||Processa({||MTC002(1)},"Processando...")},065,012,,,,.T.,,"",,,,.F. )
		oDet       := TButton():New( 200,080,"Detalhes do Calendario"	,oGrp1	,{||RoZA1()   							   },065,012,,,,.T.,,"",,,,.F. )
	...
	User Function GeraTurn()
		...
		If Pergunte(cPerg,.t.)
			Processa({||MTC002(2)},"Processando...")
		Endif

	Return()

    Static Function MTC002(nTP)
    	Local mSQL := ""
    	Local vSeq    
	...
	Static Function MTC001(vTurno,cSeqPa)   
		Local nDia		:=0
		Local nDat		:=0
		Local nC
		Local cDt 
...
//			AADD( aRegs, {cPerg  ,"04","Data Final   :"        ,"","","MV_CH04"  ,"D",08,00,0,"G","", "MV_PAR04",""              ,"","","","",""                ,"","","","",""       ,"","","","","","","","","","","","","",""   } )
	...
	User Function MTCalTur(cTurno)
		...
		mSql := " Select ZA1_TURNO,SUM(ZA1_HRTOTA)ZA1_HRTOTA " 
		mSql += " From "+RetSQLName("ZA1") 
		mSql += " WHERE ZA1_FILIAL='"+xFilial("ZA1")+"' AND D_E_L_E_T_<> '*' "

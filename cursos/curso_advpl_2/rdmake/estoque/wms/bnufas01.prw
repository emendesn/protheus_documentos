...
@ 01, 00 VTSay PadR('Pre-Nota ' , VTMaxCol())
...
_cQry := " SELECT "
_cQry += " * "
_cQry += " FROM   "+RetSqlName("SF1")+" AS SF1 (nolock)"
...
QRYA->(dbGoTop())
If QRYA->(eof())
	DLVTAviso('', 'PRE-NOTA NAO ENCONTRADA OU JA CLASSIFICADA!')
Else
...
_cQry += " ON (AA3_FILIAL='"+xFilial("AA3")+"' AND D1_DOC=AA3_XNFENT AND D1_SERIE=AA3_XSEREN "
_cQry += " AND D1_COD=AA3_CODPRO AND AA3_NUMSER='"+_cNumSer+"' AND AA3_CODCLI='"+_cFornece+"' "
_cQry += " AND AA3_LOJA='"+_cLoja+"' AND AA3.D_E_L_E_T_ = '') "
_cQry += " WHERE SF1.D_E_L_E_T_ = '' "
_cQry += " AND F1_FILIAL='"+xFilial("SF1")+"' "
_cQry += " AND F1_DOC='"+_cNF+"' "
...
Else
	...
	_cQryAA3 += " AND AA3_CODCLI='"+_cFornece+"' "
	_cQryAA3 += " AND AA3_LOJA='"+_cLoja+"' "
	...
	If QRYAA3->QTDAA3 < QRYB->QTDE
		...
		Aadd(aCabAA3, { "AA3_CODCLI"	, _cFornece      	, NIL } )
		Aadd(aCabAA3, { "AA3_LOJA"	 	, _cLoja	  		, NIL } )
		...
		Aadd(aCabAA3, { "AA3_STATUS"	, "02"				, NIL } )//ESTOQUE
	...
	Else
		DLVTAviso('', 'QTDE LIDA DO PRODUTO '+Alltrim(_cProduto)+' EXCEDE A QTDE EXISTENTE NA PRE-NOTA!')
		_cProduto := Space(15)
		_lRet := .F.
	Endif
	QRYAA3->(dbCloseArea())
...
_cQry += " AND AA3_CODCLI='"+_cFornece+"' "
_cQry += " AND AA3_LOJA='"+_cLoja+"' "
...
If !dbSeek(xFilial("SB1")+_cProduto)
	dbSelectArea("SA5")
	dbSetOrder(8)
	If dbSeek(xFilial("SA5")+_cFornece+_cLoja+_cProduto)
		...
		Endif
	Else
		DLVTAviso('', 'NรO EXISTE AMARRACAO DO PRODUTO '+Alltrim(_cProduto)+' E FORNECEDOR '+Alltrim(_cFornece))
		_cProduto := Space(15)
		_lRet 	  := .F.
		VTKeyBoard(chr(20))
	Endif
...
QRYD->(dbGoTop())
If QRYD->(eof())
	DLVTAviso('', 'PRODUTO NAO ENCONTRADO NA PRE-NOTA - '+_cNF)
	_lRet 	  := .F.
	_cProduto := Space(15)
	VTKeyBoard(chr(20))
Endif

...
User Function tecx011a()
	...
	@ 103,055 Get mv_par04 Size 024,010 Picture "999999999"  											   				Valid(u_tecx011b(mv_par04,"5")) 											Object oNota
	@ 103,122 Get mv_par05 Size 010,010 Picture "!!!"
	...
	While !(cAliasTop1)->(EOF())
		...
		If ZZ4->(DbSeek(xFilial("ZZ4") + (cAliasTop1)->ZZ4_IMEI + "1"))
			...
			While ZZ4->(!eof()) .and. ZZ4->ZZ4_IMEI == (cAliasTop1)->ZZ4_IMEI .and. ZZ4->ZZ4_STATUS == "1"  // Adicionado para solucao da gravacao do item reincidente. (11/05/05) Nicoletti
				...
				If upper(substr(ZZ4->ZZ4_EMUSER,1,13)) == upper(substr(cUserName,1,13)) //.and. ZZ4->ZZ4_STATUS == "1"
...
<<<<<<< .mine
				...
				//ÚÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄ¿
				//³ M.Munhoz - 23/05/2012 - Ticket 6361                                     ³
				//³ Alerta responsavel MCLAIM para produtos sem APCCODE na entrada massiva  ³
				//ÀÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÙ
				_cAPCCode := Posicione("SB1",1,xFilial("SB1")+ZZ4->ZZ4_CODPRO,"B1_XCODAPC")
				if empty(_cAPCCode) .and. Posicione("ZZJ",1,xFilial("ZZJ")+ZZ4->ZZ4_OPEBGH,"ZZJ_LAB") == "2"
					cPara    := getmv("MV_XMCLEMA")
					cCC      := ""
					cAssunto := "MCLAIM >>> Produto sem APCCODE"
					cMsg     := "Caro responsável pelo MCLAIM, foi feita uma entrada massiva do produto "+alltrim(ZZ4->ZZ4_CODPRO)+" no IMEI: "+alltrim(ZZ4->ZZ4_IMEI)+", o qual não possui código APC CODE (B1_XCODAPC). Favor verificar antes que o MCLAIM seja gerado."
...
>>>>>>> .r1344
			...
			if _cVconre == "S"    // Validação de Importação anterior - Luis Carlos
				dbSelectArea("SZP")
				dbSetOrder(2)
				if SZP->(dbSeek((cAliasTop1)->ZZ4_IMEI))
					RecLock('SZP',.F.)
					SZP->ZP_ENTRMAS := "S"
					msUnlock("SZP")
				endif
	...
	if ZZ4->(DBSeek(xFilial('ZZ4')+cCODBAR))
		if ZZ4->ZZ4_STATUS <> '9' // NFS gerada
			...
			while ZZ4->(!eof()) .and. ZZ4->ZZ4_FILIAL == xFilial("ZZ4") .and. alltrim(ZZ4->ZZ4_IMEI) == alltrim(cCodBar)
				...
				IF ZZ4->ZZ4_FILIAL == xFilial("ZZ4") .and. alltrim(ZZ4->ZZ4_IMEI) == alltrim(cCodBar)  .and. ZZ4->ZZ4_STATUS='9'
					_nVez++
					_nQualiVez++
...
If _lnfcomp .AND. cinfcaut='S'
	...
	@ 065,057 Get mv_par18 Size 025,010 Picture "@!"		 valid !vazio()	 Object onfcom

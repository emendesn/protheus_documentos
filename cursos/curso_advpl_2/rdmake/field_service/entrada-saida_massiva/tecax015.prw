...
cQuery := " SELECT ZZ4_FILIAL,ZZ4_CODCLI,ZZ4_LOJA,ZZ4_NFENR, ZZ4_NFESER,ZZ4_IMEI,ZZ4_OS,ZZ4_STATUS,ZZ4_PV,ZZ4_SMUSER,ZZ4_OPEBGH," + CRLF
cQuery += " ZZ4_CODPRO,ZZ4_VLRUNI,ZZ4_LOCAL,ZZ4_OPER,ZZ4_GVSOS,ZZ4_GVSARQ,ZZ4_FASATU, ZZ4_ITEMD1,ZZO_NUMCX " + CRLF
cQuery += " FROM "+ RETSQLNAME("ZZ4")+" ZZ4 (nolock) " + CRLF
cQuery += " LEFT JOIN " + RetSqlName("ZZO") + " ZZO (NOLOCK) " + CRLF
cQuery += " ON (ZZO_IMEI=ZZ4_IMEI AND ZZO_CARCAC=ZZ4_CARCAC AND ZZO_NUMCX=ZZ4_ETQMAS AND ZZO_OSFILI=ZZ4_OS AND ZZO_STATUS IN ('P','E') AND ZZO.D_E_L_E_T_ ='') " + CRLF
cQuery += " WHERE ZZ4_FILIAL  = '"+xFilial("ZZ4")+"' AND " + CRLF
cQuery += "      ZZ4_STATUS  = '7' AND " + CRLF  // OS Encerrada
cQuery += "      ZZ4_PV      = ''  AND " + CRLF
cQuery += "      ZZ4.D_E_L_E_T_  = ''  AND " + CRLF
cQuery += "      SUBSTRING(ZZ4_SMUSER,1,13) = '"+Substr(cUserName,1,13)+"' " + CRLF
cQuery += "ORDER BY ZZ4_CODCLI, ZZ4_LOJA, ZZ4_NFENR, ZZ4_NFESER" + CRLF // MUNHOZ 03/08/07
	...
	If SchZZ3Swp(@aImNovo) .and. lSwpPri .and. !Empty(cSwpCli)
		...
		If Len(aImNovo) > 0 
			For nx:=1 To Len(aImNovo) 								
				_cMenSC5 := Alltrim("IMEI NOVO " + aImNovo[nx][1] + " SI " + aImNovo[nx][2] + ". " + " IMEI ANTIGO " + Alltrim((cAliasTop5)->ZZ4_IMEI)+ ". ")	
			Next nx
		EndIf		
	Else
	...
	*/
	...
	WHILE &_cFiltro
		...
		If _lAchouSB6 //Incluso variavel _lAchouSB6 para retirar o loop // Edson Rodrigues - 14/06/10
			...
			Aadd(_aIteSC6,{	{"C6_ITEM"		,cItens	  								,Nil},;//1
			{"C6_PRODUTO"	,_cCodPro 				   								,Nil},;//2
			{"C6_DESCRI"	,SB1->B1_DESC	 		  								,Nil},;//3
			{"C6_TES"		,cTes													,Nil},;//4
			{"C6_QTDVEN"	,1						  								,Nil},;//5
			{"C6_QTDLIB"	,iif(_clibPvS="S", 1, 0) 								,Nil},;//6
			{"C6_NUMSERI"	,(cAliasTop5)->ZZ4_IMEI	  								,Nil},;//7
			{"C6_NUMOS"		,_cNumOsTk				  								,Nil},;//8
			{"C6_PRCVEN"	,_nPrcVen    			  								,Nil},;//9
			{"C6_VALOR"		,_nPrcVen 				  								,Nil},;//10
			{"C6_PRUNIT"	,0						  								,Nil},;//11
			...
			{"C6_LOCAL"		,IIF(EMPTY(_citlocal),SB1->B1_LOCPAD,_citlocal)         ,Nil},;//17
			{"C6_ENTREG"	,dDataBase				  								,Nil},;//18
			{"C6_OSGVS"		,(cAliasTop5)->ZZ4_GVSOS  								,Nil},;//19
			{"C6_AIMPGVS"	,(cAliasTop5)->ZZ4_GVSARQ 								,Nil},;//20
			{				,	                      								,Nil},;//21
			{"C6_XIMEOLD"	,Iif(lPVSwp,(cAliasTop5)->ZZ4_IMEI,"")	  				,Nil},;//22
			{"C6_IMEINOV"	,Iif(lPVSwp,cImeiNovo,"") 				  				,Nil}})//23
...
//----------------------------------------------------------------------------------------------------------------------
		...
		If !lRf //Implantacao Radio Frequencia
			...
			If _lRet
				If !lPVSwp
					aadd(aPedido,{SC5->C5_NUM,aB6KIT,.T.})
...
If (_cgeraNF == "S" .and. _clibPvS == "S"  .and.  !lPVSwp) .OR.  (lPVSwp .and. cgernfsw $ "2/3") .OR. (lPVSwp .and. cgernfsw $ "1/3"  .AND. Len(aC5SwpCab) > 0 .and. Len(aC6SwpIt) > 0)
	...
	If ApMsgYesNo("Deseja gerar NF?","Nota Fiscal")
		...
		If Len(aPedido) > 0 
			...
			//Imprime Etiqueta dos Correios com as Notas Fiscais geradas - Vinicius Leonardo - Delta Decisใo 21/07/15
			If len(aMassNF) > 0 .and. !lRf
				U_OperSedex(aMassNF)
			EndIf			
		...
		If Posicione("SF4",1,xFilial("SF4") + cTesOp, "F4_PODER3") <> "N" 
			...
			If ( _cAlias )->( !Eof() )
				While ( _cAlias )->( !Eof() ) 
					...
					Else
						lAtend := .F. 
						cPed := cPed+GetPed(( _cAlias )->B6_IDENT, ( _cAlias )->B6_SALDO)						   
					Endif
					If lAtend 
						Exit
					EndIf	
...
EndIf
//Inclusao do PV
IF lvldsld .and. lAtend
    lRet := u_geraPV(aC5SwpCab, aC6SwpIt, 3, .T.)      
     If lRet     
	     aadd(aPedido,{SC5->C5_NUM,{},.T.})
     Endif
Else
	If !Empty(cPed)
		If !lRf //Implanta็ใo radio frequencia
			ApMsgInfo("Nใo foi possํvel localizar saldo de terceiros para o produto " + cmodNovo + " , pois jแ existe um ou mais pedidos em aberto com o saldo empenhado: "+cPed+"."+CRLF+"Por favor, contate a Controladoria e solicite anแlise.","Saldo insuficiente de Terceiros")
			lvldsld := .f.
		Else
			VTBEEP(3)
			VTAlert("Nใo foi possํvel localizar saldo de terceiros para o produto " + cmodNovo + " , pois jแ existe um ou mais pedidos em aberto com o saldo empenhado: "+cPed+"."+CRLF+"Por favor, contate a Controladoria e solicite anแlise.","Saldo insuficiente de Terceiros",.t.,2500)
			lvldsld := .f.
		Endif
	EndIf	      
ENDIF
...
฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿
*/
Static Function GetPed(_cIdent)
...
cQuery := " SELECT C6_NUM " + CTRL
cQuery += " FROM "+RetSqlName("SC6")+" (NOLOCK) WHERE C6_FILIAL='"+xFilial("SC6")+"'  " + CTRL 
cQuery += " AND C6_IDENTB6 = '"+_cIdent+"' AND D_E_L_E_T_ <> '*' AND C6_NOTA = '' " + CTRL 

TCQUERY cQuery ALIAS "PVC6" NEW
dbSelectArea("PVC6")

While PVC6->(!EOF())
	cNumPed := cNumPed+PVC6->C6_NUM+"|" 
	PVC6->(DbSkip())
EndDo
...
cQuery += " ORDER BY ZZ3.R_E_C_N_O_ " + CRLF 
	...
	If SZ9->Z9_STATUS == "1" .AND. !Empty(SZ9->Z9_PARTNR) .AND. SZ9->Z9_USED="0".AND. !Empty(cTESPecs)  .AND.  Empty(SZ9->Z9_PEDPECA)
		...
		If SB1->(DBSeek(xFilial("SB1")+cpartn)) //.and. carmpc $ left(carmense,2) desabilitado ate comecarem o apontamento correto, informando o armazem.
			...
			If !SB2->(DBSeek(xFilial("SB2")+SB1->B1_COD+carmpc))
				U_saldoini(SB1->B1_COD,alltrim(carmpc),.T.)
			Endif
			...
			If  cestq=="S"
		        ...
		        Elseif _cvalisald == "NOTSB2"
       				 MsgInfo("Sem saldo suficiente para o PN: "+alltrim(SB1->B1_COD)+", nใo sera possivel gerar a NF do PV de Pe็as. O PV pe็as ficarแ bloqueado para analise e libera็ใo.","Saldo Insuficiente")
       				 U_saldoini(SB1->B1_COD,alltrim(carmpc),.F.)
                     cendaud:=""
                     _clibPvS :="N"
			    Endif      
			Else
...
//For nXEmezenc:=1 to len(_aitpcSC6)
	...
	//Conforme acordado com Edson , comentado para gerar linha a linha de item para ter a referancia das OS's - Vinicius Leonardo - Delta Decisao - 13/08/2015
	//IF _cproduct<>_cnewprod
		...
		For nYEmezenc := 1 to len(_aitpcSC6) ////Conforme acordado com Edson , comentado para gerar linha a linha de item para ter a referancia das OS's - Vinicius Leonardo - Delta Decisao - 13/08/2015
			//IF _aitpcSC6[nYEmezenc,22,2]==_cnewprod
				//IF  (_aitpcSC6[nYEmezenc,22,2]+_aitpcSC6[nYEmezenc,2,2]<>_cpropeca) .or. ( _aitpcSC6[nYEmezenc,9,2]<>_nprcven)  
					If !lPVSwp
						If _nPosPVRet > 0
							_aCPCSC5[_nPosPVRet,2] := _aitpcSC6[nYEmezenc,25,2]
						EndIf
					EndIf	
		...
		If len(_aCPCSC5) > 0  .and. len(_aitpcprod) > 0
			...
			//Endif
			If _lRet
				aadd(aPedido,{SC5->C5_NUM,aB6KIT,.F.})
...
cNrMass:= GETSXENUM("SF2","F2_XMASSIV") 
ConfirmSX8()                                                                                                       
	...
	If Empty(aBloqueio) .And. !Empty(aPvlNfs)
		...
		For nX := 1 To Len(aNotas)
			...
			If aPedido[1,3]
				...
				aADD(aMassNF,{SF2->F2_SERIE,SF2->F2_DOC,SF2->F2_CLIENTE,SF2->F2_LOJA})
			EndIf
		Next nX
	Endif
	AtuSF2(aAuxNF)// grava numero de etiqueta na NF 		 
	AtuMass(aMassNF)// grava numero da massiva na NF 		 
Else
	For i:=1 to len(aPedido)
		aPvlNfs   := {}
		aBloqueio := {{"","","","","","","",""}}
		...
		If dbSeek(xFilial("SC5")+aPedido[i,1])
			...
			If Empty(aBloqueio) .And. !Empty(aPvlNfs)
				...
				Else
					If !lPVSwp						
						GrvPecRet()// grava o pedido de pe็a no pedido de retorno					
						GrvMNFPC()	// grava na mensagem para nota do pedido de pe็a, a nota de do pedido de retorno					
					EndIf	
					// Efetua as quebras de acordo com o numero de itens
					For nX := 1 To Len(aPvlNfs)
						If Len(aNotas[Len(aNotas)])>=nItemNf
...
฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿
*/
Static Function GrvMNFPC() 

	Local aArea  := GetArea()
	Local cQuery := ""
	
	cQuery := " SELECT C5_NOTA, C5_SERIE FROM " + RetSqlName("SC5") + CRLF
	cQuery += " WHERE C5_PVPEC = '" + SC5->C5_NUM + "' " + CRLF
	cQuery += " AND D_E_L_E_T_ <> '*' AND C5_FILIAL = '" + xFilial("SC5") + "' " + CRLF	
	If Select("PCPEC") > 0
		PCPEC->(dbCloseArea())
	EndIf	
	dbUseArea(.T., "TOPCONN", TCGenQry(,, cQuery), "PCPEC", .F., .T.)	
	dbSelectArea("PCPEC")
	PCPEC->(dbGoTop())	
	If PCPEC->(!Eof()) 
		If RecLock("SC5",.F.)
			SC5->C5_MENNOTA := Alltrim(SC5->C5_MENNOTA)+" NF: " + Alltrim(PCPEC->C5_NOTA) + " SERIE: " + Alltrim(PCPEC->C5_SERIE)
			SC5->(MsUnlock())
		EndIf	
	EndIf	
	RestArea(aArea)
...
ฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑ
ฑฑษออออออออออัออออออออออหอออออออัออออออออออออออออออออหออออออัอออออออออออออปฑฑ
ฑฑบPrograma  ณAtuMass   บAutor  ณ                    บ Data ณ    /  /     บฑฑ
ฑฑฬออออออออออุออออออออออสอออออออฯออออออออออออออออออออสออออออฯอออออออออออออนฑฑ
ฑฑบDesc.     ณ                                                            บฑฑ
ฑฑบ          ณ                                                            บฑฑ
ฑฑฬออออออออออุออออออออออออออออออออออออออออออออออออออออออออออออออออออออออออนฑฑ
...
฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿
*/
Static Function AtuMass(aMassNF)
	...
	SF2->(DbSetOrder(2))	
	If Len(aMassNF) > 0
		For nx:=1 To Len(aMassNF)	 
			SF2->(DbGoTop())			
			If SF2->(DbSeek(xFilial("SF2")+aMassNF[nx][3]+aMassNF[nx][4]+aMassNF[nx][2]+aMassNF[nx][1]))				
				If RecLock("SF2",.F.)
					SF2->F2_XMASSIV := cNrMass
					SF2->(MsUnlock())
				EndIf
			EndIf 
		Next nx
...
While TSQLSZ9->(!Eof())
	...
	IF ltesp3
		...
		*/
			...
			/* Bloco comentado pois estแ fazendo uma busca em uma tabela defasada, onde nใo atende mais as necessidades.
				Vinicius Leonardo - Delta Decisใo - 28/07/2015
...
Local aAreaAtu := GetArea()
Local cTSPeca  :=Posicione("ZZJ",1,xFilial("ZZJ") + _coperbgh, "ZZJ_TSPECA") 
Local cAlmScr  :=Posicione("ZZJ",1,xFilial("ZZJ") + _coperbgh, "ZZJ_ALMSCR")
...
For i:=1 To Len(aPVZZQ)	
	...
	cQryPec += " C6_IDENTB6 AS IDENT, C6_CLI AS CLISAI, C6_LOJA AS LOJSAI, C6_NUMOS AS NUMOS " + CRLF
	...
	While TSQLPEC->(!Eof())
		...
		cQrySC6 += " AND LEFT(C6_NUMOS,6)='"+TSQLPEC->NUMOS+"' " + CRLF		
